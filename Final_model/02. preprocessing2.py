...
def select_pum2(df, pum, time):
    ...
    df_apple= forcast_week(df_apple, 2) # 2주일 뒤의 가격 예측
...
def pre_2(df, pum, time) : 
    ...
    # train, test split
    X_train,y_train,X_test,y_test=split_train_and_test(df_pum,'2019-09-22',2)
...
def modeling2(df, pum, time, x_columns) :
    ...
    plt.ylim(0, 1)
    plt.title(f'{pum} 14일 예측 MAPE', fontsize=15)
    plt.show()
