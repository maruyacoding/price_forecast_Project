...
def select_pum2(df, pum, time):
    ...
    df_apple= forcast_week(df_apple, 4) # 4주일 뒤의 가격 예측
...
def pre_2(df, pum, time) : 
    ...
    # train, test split
    X_train,y_train,X_test,y_test=split_train_and_test(df_pum,'2019-09-22',4)
    print(X_train.shape, y_train.shape, X_test.shape, y_test.shape)

    x_columns = list(df_pum.columns)
    x_columns.remove('SALEDATE')
    x_columns.remove('4week')
    x_columns
...
def modeling2(df, pum, time, x_columns) :
    ...
    plt.ylim(0, 1)
    plt.title(f'{pum} 4주 뒤 가격 예측 MAPE', fontsize=15)
    plt.show()
